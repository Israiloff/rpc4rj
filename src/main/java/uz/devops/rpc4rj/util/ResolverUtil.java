...
public class ResolverUtil {
    ...
    public List<JsonRpcInfo> getRpcInfoList() {
    ...
    public List<JsonRpcInfo> getRpcInfoListByMethod(Class<?> service) {
        ...
        return Arrays
                ...
                .map(method -> getJsonRpcInfo(bean, method))
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
    }

    public JsonRpcInfo getJsonRpcInfo(Object bean, Method method) {
        ...
        return methodAnnotation == null ? null : new JsonRpcInfo(bean, methodAnnotation.value(), method, params);
    }
    ...
    public boolean isDesiredMethod(@NotNull JsonRpcRequest request, JsonRpcInfo rpcInfo) {
    ...
    public Object executeMethod(JsonRpcRequest request, JsonRpcInfo jsonRpcInfo)
            throws IllegalAccessException, InvocationTargetException, InvalidParamsException {
        log.trace(
                "executeMethod started for method : {}, of type : {}",
                jsonRpcInfo.getMethodName(),
                jsonRpcInfo.getInstance().getClass().getSimpleName()
        );

        var params = ((LinkedHashMap<String, ?>) request.getParams());

        if (params.size() != jsonRpcInfo.getParams().size()) {
            throw new InvalidParamsException();
        }

        var args = jsonRpcInfo
                .getParams()
                .stream()
        ...
        return jsonRpcInfo.getMethod().invoke(jsonRpcInfo.getInstance(), args);
    }
    ...
    public Mono<ResponseEntity<?>> mapResult(JsonRpcRequest request, JsonRpcInfo jsonRpcInfo, Object result)
            throws InvalidWrapperException {
        log.trace("mapResult started result of method : {}", jsonRpcInfo.getMethodName());

        if (jsonRpcInfo.getMethod().getReturnType().equals(Mono.class)) {
            log.trace("result is Mono type");
            return ((Mono<?>) result).map(response -> ResponseEntity.ok(mapToJsonRpcResponse(request, response)));
        }

        if (jsonRpcInfo.getMethod().getReturnType().equals(Flux.class)) {
